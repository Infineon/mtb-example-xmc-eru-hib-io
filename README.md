# XMC&trade; MCU: ERU HIB IO

This code example demonstrates the routing of the hibernate pin state through the Event Request Unit (ERU) block when powered in hibernate domain for XMC&trade; MCU devices.

In this code example, the LED blinks at 500 ms. On changing the state of the HIB-IO pin, the LED blinking halts; it resume when the HIB-IO pin state is released.
...
The following example will clone the "[ERU_HIB_IO](https://github.com/Infineon/mtb-example-xmc-eru-hib-io)" application with the desired name "ERU_HIB_IO" configured for the *KIT_XMC43_RELAX_ECAT_V1* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_XMC43_RELAX_ECAT_V1 --app-id mtb-example-xmc-eru-hib-io --user-app-name ERU_HIB_IO --target-dir "C:/mtb_projects"
   ```
...
3. Confirm that the kit LED blinks at approximately 500 ms.

4. Trigger an event via the HIB-IO pin as follows for the specific kits and observe that the LED halts blinking.

   - XMC4300 Relax EtherCAT kit: Press Button2(HIB1) as the trigger event.

   - XMC4200 Platform2Go kit: Connect a jumper wire from the HIB_IO_0 pin to 3.3V as the trigger event.

   - XMC4400 Platform2Go kit, XMC4500 relax kit, and XMC4800 relax EtherCAT kit: Connect a jumper wire from the HIB_IO_1 pin to GND as the trigger event.
...
The application uses the HIB-IO pin configured as an input to the ERU block. ERU0_ETL1 is configured with falling edge detection and automatic clearing of the ERU status flag. This event detection on the HIB_IO pin triggers the blinking LED to halt.
...
Document title: *CE236875* – *XMC&trade; MCU: ERU HIB IO*
